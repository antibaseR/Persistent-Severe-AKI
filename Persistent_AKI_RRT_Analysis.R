...
# setting
for (analysis in c("_RRT", "_RRT_with_death")){
  ...
  for (i in 1:length(impDat)){
    ...
    if (analysis == "_RRT" | analysis == "_RRT_with_death"){
      data = data %>%
    ...
    completeData = completeData %>%
      mutate(apache_AKI = apache3 * (as.numeric(AKI_Category_final)-1))
