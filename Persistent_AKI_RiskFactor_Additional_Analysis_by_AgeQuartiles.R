...
for (analysis in c("_additional_ageq", "_additional_ageq_sensitivity")){
  ...
  for (i in 1:length(impDat)){
    ...
    if (analysis=="_additional_ageq_sensitivity"){
      subdata = data %>%
    ...
    for (ageq in 1:4){
      ...
      # include interaction
      completeData_sub = completeData_sub %>%
        mutate(DIC_CARDIAC = (as.numeric(DISSEMINATED_INTRAVASCULAR_COAGULATION)-1) * (as.numeric(CARDIAC_ARREST)-1))
      train = completeData_sub[trainIndex,]
      test = completeData_sub[-trainIndex,]
      ...
      logi.model = train(x = trainData,
                         ...
                         trControl = control,
                         metric = "ROC")
      train_sum_list[[ageq]] = logi.model
      var_imp_list[[ageq]] = varImp(logi.model)
      
      # evaluating on the testing set
      pred_prob = predict(logi.model, newdata = testData, type = "prob")
      test_sum_list[[ageq]] = roc(testLabel, pred_prob$Yes) # Draw ROC curve.
  ...
  for (j in 1:4){
    ...
    write.csv(AUC_df, paste("G:/Persistent_AKI/Xinlei/Result/AUC_df", analysis, "_", j, ".csv", sep=""))
    ...
    beta_df$variable_true = c("Intercept",
                              ...
                              "Acute respiratory failure: yes",
                              "Apache 3",
                              "Cardiac arrest: yes",
                              "Cardiogenic shock: yes",
                              "CVA: yes",
                              "Charlson",
                              "DIC: yes * Cardiac arrest: yes",
                              "DIC: yes",
                              "DVT: yes",
                              "Gender: male",
                              "Hemorrhage: yes",
                              "Left right biventricular failure: yes",
