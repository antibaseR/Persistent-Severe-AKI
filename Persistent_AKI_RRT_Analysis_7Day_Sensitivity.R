...
# setting
for (analysis in c("_RRT_7day_sensitivity", "_RRT_7day_sensitivity_with_death")){
  ...
  for (i in 1:length(impDat)){
    ...
    if (analysis == "_RRT_7day_sensitivity" | analysis == "_RRT_7day_sensitivity_with_death"){
      data = data %>%
    ...
    if (analysis == "_RRT_7day_sensitivity"){
      data = data %>%
        mutate(rrt_discharge = factor(ifelse(rrt_discharge==1, "Yes", "No")))
    }
    
    if (analysis == "_RRT_7day_sensitivity_with_death"){
      data = data %>%
    ...
    if (analysis=="_RRT_7day_sensitivity" | analysis == "_RRT_RRT_7day_sensitivity_with_death"){
      subdata = subdata %>%
        filter(!(difftime(death_date_sh_true, dt_icu_start_sh, units="days")<7))
    }
    ...
    completeData = completeData %>%
      mutate(refCreat_AKI = refCreat * (as.numeric(AKI_Category_final)-1))
  ...
  beta_df$variable_true = c("Intercept",
                            ...
                            "Race: none-white",
                            "Reference creatine",
                            "Reference creatine * Persistent severe AKI: yes")
