...
for (analysis in c("_survival_sensitivity")){
  ...
  for (i in 1:length(impDat)){
    ...
    subdata = subdata[, c(names(subdata) %in% surv_var)] %>%
    ...
    kmdata = subdata %>% 
      select(survival_time, dead_90, AKI_Category_subgroup) %>%
  ...
  km_fit = survfit(surve_obj ~ AKI_Category_subgroup, data = kmdata)
  km_plot = ggsurvplot(km_fit, data = kmdata,
                       ...
                       legend.labs = c("No AKI", "Stage 1 AKI", "No Persistent Severe AKI", "Persistent Severe AKI"),    # Change legend labels
                       risk.table.height = 0.25, # Useful to change when you have multiple groups
                       ggtheme = theme_bw()
  )
  pdf(paste("G:/Persistent_AKI/Xinlei/Result/KM_Plot/km_plot", analysis, ".pdf", sep=""), width = 15, height = 8)
  print(km_plot$plot, newpage = FALSE)
  dev.off()
  logrank_fit = survdiff(surve_obj ~ AKI_Category_subgroup, data=kmdata)
  ...
  for (i in 1:length(pair_list)){
    pair = pair_list[[i]]
    kmdata_sub = kmdata %>% filter(AKI_Category_subgroup %in% pair)
    surve_obj = Surv(kmdata_sub$survival_time, kmdata_sub$dead_90)
    km_fit = survfit(surve_obj ~ AKI_Category_subgroup, data = kmdata_sub)
    km_plot = ggsurvplot(km_fit, data = kmdata_sub,
                         ...
                         risk.table.height = 0.25, # Useful to change when you have multiple groups
                         ggtheme = theme_bw())
    pdf(paste("G:/Persistent_AKI/Xinlei/Result/KM_Plot/km_plot_", pair[1], "_", pair[2], analysis, ".pdf", sep=""), width = 15, height = 8)
    print(km_plot$plot, newpage = FALSE)
    dev.off()
    logrank_fit = survdiff(surve_obj ~ AKI_Category_subgroup, data=kmdata_sub)
