...
for (i in 1:length(impDat)){
  ...
  if (analysis == "_vaso_intervention"){
    data = data %>%
  ...
  if (analysis=="_vaso_intervention_sensitivity"){
    subdata = data %>%
  ...
  for (q in 1:nrow(subdata)){
    if (subdata$TOTAL_INPUTS_BEFORE_AKI[q] >= Q0 & subdata$TOTAL_INPUTS_BEFORE_AKI[q] < Q1){
      subdata$fluid_quartiles[q] = "1"
    } else if (subdata$TOTAL_INPUTS_BEFORE_AKI[q] >= Q1 & subdata$TOTAL_INPUTS_BEFORE_AKI[q] < Q2){
      subdata$fluid_quartiles[q] = "2"
    } else if (subdata$TOTAL_INPUTS_BEFORE_AKI[q] >= Q2 & subdata$TOTAL_INPUTS_BEFORE_AKI[q] < Q3){
      subdata$fluid_quartiles[q] = "3"
    } else if (subdata$TOTAL_INPUTS_BEFORE_AKI[q] >= Q3 & subdata$TOTAL_INPUTS_BEFORE_AKI[q] <= Q4){
      subdata$fluid_quartiles[q] = "4"
    }
  }
  
  subdata$fluid_quartiles = factor(subdata$fluid_quartiles, levels = c("1", "2", "3", "4"))
...
beta_df$variable_true = c("Intercept",
                          "Age at admission",
                          "Apache 3",
                          "Total fluid input before AKI: 2nd quartile",
                          "Total fluid input before AKI: 3rd quartile",
                          "Total fluid input before AKI: 4th quartile",
                          "Gender: male",
