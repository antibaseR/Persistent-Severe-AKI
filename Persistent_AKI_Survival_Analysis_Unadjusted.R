...
#analysis = c("_survival_unadjusted_sensitivity")
...
for (i in 1:length(impDat)){
  ...
  if (analysis == "_survival" | analysis == "_survival_unadjusted"){
    data = data %>%
  ...
  # subset the data
  if (analysis=="_survival_unadjusted_sensitivity"){
    subdata = data %>%
  ...
  coxdata = subdata[, c(names(subdata) %in% adj_var)] %>%
    ...
    mutate(AKI_Category_true = factor(AKI_Category_true, levels=c("Transient_AKI", "Persistent_AKI")))
