...
# setting
analysis="_renal_recovery_unadjusted"
...
for (i in 1:length(impDat)){
  ...
  if (analysis == "_renal_recovery" | analysis == "_renal_recovery_unadjusted"){
    data = data %>%
  ...
  trainData = as.data.frame(train[, -(which("renal_recov"==colnames(train)))])
  colnames(trainData) = colnames(train)[-(which("renal_recov"==colnames(train)))]
  testLabel = test$renal_recov
  testData = as.data.frame(test[, -(which("renal_recov"==colnames(test)))])
  colnames(testData) = colnames(test)[-(which("renal_recov"==colnames(test)))]
