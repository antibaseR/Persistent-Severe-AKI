...
# setting
analysis = "_RRT_subgroup_with_death"
#analysis="_RRT_subgroup"
# analysis="_RRT_subgroup_sensitivity"
...
for (i in 1:length(impDat)){
  ...
  if (analysis == "_RRT_subgroup" | analysis == "_RRT_subgroup_with_death"){
    data = data %>%
  ...
  data = data %>%
    ...
    mutate(AKI_Category_subgroup = factor(AKI_Category_subgroup, levels=c("Transient Stage 1",
                                                                          "Persistent Stage 1",
                                                                          "Transient-Transient Stage 2",
                                                                          "Transient-Persistent Stage 2",
                                                                          "Transient-Transient Stage 3",
                                                                          "Transient-Persistent Stage3",
                                                                          "Persistent_AKI")))
  # recode race
  data = data %>%
    mutate(race = ifelse(race==1, 0, 1))
  
  if (analysis == "_RRT_subgroup"){
    data = data %>%
      mutate(rrt_discharge = factor(ifelse(rrt_discharge==1, "Yes", "No")))
  }
  
  if (analysis == "_RRT_subgroup_with_death"){
    data = data %>%
  ...
  if (analysis=="_RRT_subgroup_sensitivity"){
    subdata = data %>%
...
beta_df$variable_true = c("Intercept",
                          "Age at admission",
                          "Persistent stage 1 AKI: yes",
                          "Persistent severe AKI: yes",
                          "Persistent stage 2 AKI: yes",
                          "Persistent stage 3 AKI: yes",
                          "Transient stage 2 AKI: yes",
                          "Transient stage 3 AKI: yes",
                          "Apache 3",
                          "Charlson",
                          "Gender: male",
