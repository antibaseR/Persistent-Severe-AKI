...
# setting
for (analysis in c("_no_sepsis_no_cardiac_surgery", "_no_sepsis_no_cardiac_surgery_sensitivity")){
  ...
  for (i in 1:length(impDat)){
    ...
    #only include patients with no sepsis and no cardiac surgery
    data = data %>%
      filter(CARDIAC_SURG==0 & SEPSIS ==0)
    ...
    nrow(data)
    if (analysis=="_no_sepsis_no_cardiac_surgery_sensitivity"){
      subdata = data %>%
  ...
  saveRDS(caret_mod_list, paste("G:/Persistent_AKI/Xinlei/Persistent_AKI_Additional_Analysis/Data/caret_mod_list", analysis, ".rds", sep=""))
  saveRDS(ensemble_mod_list, paste("G:/Persistent_AKI/Xinlei/Persistent_AKI_Additional_Analysis/Data/ensemble_mod_list", analysis, ".rds", sep=""))
