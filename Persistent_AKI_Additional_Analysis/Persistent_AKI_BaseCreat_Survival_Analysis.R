...
for (analysis in c("_BaseCreat_survival_analysis")){
  
  impDat = readRDS("G:/Persistent_AKI/Xinlei/Data/Data_Imputed.rds")
  rawData = readRDS('G:/Persistent_AKI/Xinlei/Data/Data_Revised.rds')
  creatData = read.csv('G:/Persistent_AKI/Persistent_AKI_Data/year1cr_data.csv')
  ...
  for (i in 1:length(impDat)){
    ...
    #subset data to patients with refCreat
    data = data %>%
      filter(year1cr == 1)
    ...
    #plot(get.tree(obj, 1))
    coxdata = coxdata %>%
      mutate(sepsis_apache = as.numeric(as.character(SEPSIS)) * apache3)
