...
for (analysis in c("_CompetingRisk_RRT_90")){
  ...
  for (i in 1:length(impDat)){
    ...
    check = subdata %>%
      select(death_date_sh_true, dead_true)
    
    ## Days for RRT
    subdata$rtime = subdata$rrt_days
    
    ## Construct data for Fine and Gray model
    subdata$etime = with(subdata, ifelse(rrt_90==0, dtime, rtime))
    subdata$event = with(subdata, ifelse(rrt_90==0, 2*dead_90, 1))
    subdata$event = factor(subdata$event, 0:2, labels=c("censor", "rrt_90", "death"))
    ...
    #set.seed(960725)
    #obj = rfsrc(Surv(etime, event) ~ ., coxdata,
    #ntree = 1, nodesize = 5, nsplit = 10, importance = TRUE)
    #tree = get.tree(obj, 1)
    #tree
    
    coxdata = coxdata %>%
      mutate(apache_mv = apache3* MV_DAYS)
    
    pdata = finegray(Surv(etime, event) ~ ., data=coxdata)

    cox_fit = coxph(Surv(fgstart, fgstop, fgstatus) ~ AKI_Category_subgroup + age_at_admission + 
                      gender + race + MV_DAYS + refCreat + apache3 + charlson + apache_mv,
  ...
  beta_df$variable_true = c("Age at admission",
                            ...
                            "Apache 3 * Days of mechanical ventilation",
                            "Apache 3",
                            "Charlson",
                            "Gender: male",
