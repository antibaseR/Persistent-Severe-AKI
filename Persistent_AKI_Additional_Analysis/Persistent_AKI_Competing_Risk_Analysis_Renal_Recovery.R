...
for (analysis in c("_CompetingRisk_RenalRecovery")){
  ...
  for (i in 1:length(impDat)){
    ...
    ## Days for renal recovery
    subdata$rtime = difftime(subdata$encounter_end_date_time_sh, subdata$FIRST_AKI_DATETIME, units = "days")

    ## Construct data for Fine and Gray model
    subdata$etime <- with(subdata, ifelse(renal_recov==0, dtime, rtime))
    subdata$event <- with(subdata, ifelse(renal_recov==0, 2*dead_true, 1))
    subdata$event <- factor(subdata$event, 0:2, labels=c("censor", "renal_recovery", "death"))
    ...
    adj_var = c("age_at_admission",
     ...
     pdata = finegray(Surv(etime, event) ~ ., data=coxdata)
     #obj = rfsrc(Surv(etime, event) ~ age_at_admission + gender +
     #               charlson + apache3 + refCreat + AKI_Category_subgroup, coxdata,
     #             ntree = 1, nodesize = 5, nsplit = 10, importance = TRUE)
     
     #tree = get.tree(obj, 1)
     cox_fit = coxph(Surv(fgstart, fgstop, fgstatus) ~ age_at_admission + gender +
                       charlson + apache3 + refCreat + AKI_Category_subgroup + AKI_Category_subgroup*apache3,
  ...
  beta_df$variable_true =  c("Age at admission", 
                             ...
                             "Apache 3",
                             "Apache 3 * Persistent severe AKI: yes",
                             "Charlson",
                             "Gender: male",
