...
analysis = "_subgroup_psAKI_survival_analysis"
...
for (i in 1:length(impDat)){
  ...
  if (analysis == "_subgroup_psAKI_survival_analysis"){
    data = data %>%
  ...
  # subset the data
  if (analysis=="_subgroup_psAKI_survival_sensitivity"){
    subdata = data %>%
  ...
  subdata = subdata %>% 
    ...
    mutate(psAKI = ifelse(grepl("333", KDIGO_pattern), "Yes", "No"),
           psAKI = factor(psAKI, levels = c("No", "Yes")))
