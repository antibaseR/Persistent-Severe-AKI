...
for (i in 1:length(impDat)){
  ...
  Q1_pAKI = completeData %>%
    filter(age_at_admission>=Q0 & age_at_admission<Q1) %>%
    filter(AKI_Category_final=="Yes") %>%
    nrow()
  
  Q2_pAKI = completeData %>%
    filter(age_at_admission>=Q1 & age_at_admission<Q2) %>%
    filter(AKI_Category_final=="Yes") %>%
    nrow()
  
  Q3_pAKI = completeData %>%
    filter(age_at_admission>=Q2 & age_at_admission<Q3) %>%
    filter(AKI_Category_final=="Yes") %>%
    nrow()
  
  Q4_pAKI = completeData %>%
    filter(age_at_admission>=Q3 & age_at_admission<Q4) %>%
    filter(AKI_Category_final=="Yes") %>%
    nrow()
  ...
  Q1_pAKI_list = append(Q1_pAKI_list, Q1_pAKI)
  Q2_pAKI_list = append(Q2_pAKI_list, Q2_pAKI)
  Q3_pAKI_list = append(Q3_pAKI_list, Q3_pAKI)
  Q4_pAKI_list = append(Q4_pAKI_list, Q4_pAKI)
...
mean(Q1_pAKI_list)/mean(n1_list)
mean(Q2_pAKI_list)/mean(n2_list)
mean(Q3_pAKI_list)/mean(n3_list)
mean(Q4_pAKI_list)/mean(n4_list)
