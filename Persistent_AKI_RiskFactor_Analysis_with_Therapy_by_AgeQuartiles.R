...
# setting
for (analysis in c("_with_therapy_ageq", "_with_therapy_ageq_sensitivity")){
  ...
  train_sum_list_imp = list()
  test_sum_list_imp = list()
  var_imp_list_imp = list()
  shap_val_list_imp = list()
  caret_mod_list_imp = list()
  ensemble_mod_list_imp = list()
  train_dat_list_imp = list()
  test_dat_list_imp = list()
  ...
  for (i in 1:length(impDat)){
    ...
    nrow(data)
    if (analysis=="_with_therapy_ageq_sensitivity"){
      subdata = data %>%
    ...
    for (ageq in 1:4){
      ...
      model_list = caretList(x = trainData, y = trainLabel, trControl = control, 
                             ...
                             tuneList=list(
                               ...
                               #nn=caretModelSpec(method="nnet", tuneLength=3, trace=FALSE)
                             ))
      caret_mod_list[[ageq]] = model_list
      ...
      ensemble_mod_list[[ageq]] = ensemble
      train_sum_list[[ageq]] = summary(ensemble)
      var_imp_list[[ageq]] = varImp(ensemble)
      ...
      test_sum_list[[ageq]] = caTools::colAUC(model_preds, test$AKI_Category_final)
      
      train_dat_list[[ageq]] = train
      test_dat_list[[ageq]] = test
    }
    
    train_sum_list_imp[[i]] = train_sum_list
    test_sum_list_imp[[i]] = test_sum_list
    var_imp_list_imp[[i]] = var_imp_list
    shap_val_list_imp[[i]] = shap_val_list
    caret_mod_list_imp[[i]] = caret_mod_list
    ensemble_mod_list_imp[[i]] = ensemble_mod_list
    train_dat_list_imp[[i]] = train_dat_list
    test_dat_list_imp[[i]] = test_dat_list
    
  }
  
  saveRDS(train_sum_list_imp, paste("G:/Persistent_AKI/Xinlei/Data/train_sum_list", analysis, ".rds", sep=""))
  saveRDS(test_sum_list_imp, paste("G:/Persistent_AKI/Xinlei/Data/test_sum_list", analysis, ".rds", sep=""))
  saveRDS(var_imp_list_imp, paste("G:/Persistent_AKI/Xinlei/Data/var_imp_list", analysis, ".rds", sep=""))
  saveRDS(caret_mod_list_imp, paste("G:/Persistent_AKI/Xinlei/Data/caret_mod_list", analysis, ".rds", sep=""))
  saveRDS(ensemble_mod_list_imp, paste("G:/Persistent_AKI/Xinlei/Data/ensemble_mod_list", analysis, ".rds", sep=""))
  saveRDS(train_dat_list_imp, paste("G:/Persistent_AKI/Xinlei/Data/train_dat_list", analysis, ".rds", sep=""))
  saveRDS(test_dat_list_imp, paste("G:/Persistent_AKI/Xinlei/Data/test_dat_list", analysis, ".rds", sep=""))
