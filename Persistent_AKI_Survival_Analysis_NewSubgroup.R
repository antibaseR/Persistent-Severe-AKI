...
analysis = "_survival_analysis_newsubgroup"
...
cox_pair_list = list(c("Transient_AKI", "Persistent_Severe_AKI"),
                     c("Persistent AKI stage 1", "Persistent_Severe_AKI"),
                     c("Persistent AKI stage 2", "Persistent_Severe_AKI"),
                     c("Persistent AKI stage 3", "Persistent_Severe_AKI")
                     )
...
variable_true[[3]] = c("Age at admission",
                       "Persistent severe AKI: yes",
                       "Apache 3 * Charlson",
                       "Apache 3",
                       "BMI",
...
variable_true[[4]] = c("Age at admission",
                       "Persistent severe AKI: yes",
                       "Apache 3",
                       "BMI",
                       "BMI * Charlson",
...
variable_order[[3]] = c("Age at admission",
               "Gender: male",
               "Race: non-white",
               "BMI",
               "Apache 3",
...
variable_order[[4]] = c("Age at admission",
               "Gender: male",
               "Race: non-white",
               "BMI",
               "Apache 3",
...
for (analysis in c("_survival_analysis_newsubgroup")){
  ...
  for (i in 1:length(impDat)){
    ...
    if (analysis == "_survival_analysis_newsubgroup" | analysis == "_survival_sensitivity_newsubgroup"){
      data = data %>%
    ...
    # subset the data
    if (analysis=="_survival_sensitivity_newsubgroup"){
      subdata = data %>%
    ...
    for (p in 1:length(cox_pair_list)){
      ...
      if (p==1){
        ...
        coxdata = subdata[, c(names(subdata) %in% adj_var)] %>%
          filter((AKI_Category_subgroup_new %in% cox_pair_list[[p]])) %>%
          mutate(race = ifelse(race==1, 0, 1),
                 AKI_Category_subgroup_new = factor(ifelse(AKI_Category_subgroup_new =="Persistent_Severe_AKI", "Yes", "No"), 
                                                    levels=c("No", "Yes")),
        ...
        coxdata = subdata[, c(names(subdata) %in% adj_var)] %>%
          ...
          mutate(race = ifelse(race==1, 0, 1),
                 AKI_Category_subgroup_new = factor(ifelse(AKI_Category_subgroup_new =="Persistent_Severe_AKI", "Yes", "No"), 
                                                    levels=c("No", "Yes")),
                 apache_charlson = apache3 * charlson)
