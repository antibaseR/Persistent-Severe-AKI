...
# setting
analysis_list= c("_intervention_by_each_renal_recovery_only", "_intervention_by_each_renal_recovery_death")
...
for (analysis in analysis_list){
  ...
  for (i in 1:length(impDat)){
    ...
    if(analysis %in% c("_intervention_by_each_renal_recovery_only", "_intervention_by_each_renal_recovery_death")){
      data = data %>%
    ...
    if (analysis == "_intervention_by_each_renal_recovery_only"){
      data = data %>%
        mutate(renal_recov = factor(ifelse(renal_recov==1, "Yes", "No")))
    }
    
    if (analysis == "_intervention_by_each_renal_recovery_death"){
      data = data %>%
        mutate(renal_recov = factor(ifelse((renal_recov==1|dead_hosp==1), "Yes", "No")))
    }
    ...
    for (int in 1:length(int_var)){
      ...
      set.seed(960725)
      trainIndex <- createDataPartition(completeData$renal_recov, 
                                        p = .8, 
                                        list = FALSE, 
                                        times = 1)
      ...
      trainLabel = train$renal_recov
      trainData = train[, -(which("renal_recov"==colnames(train)))]
      testLabel = test$renal_recov
      testData = test[, -(which("renal_recov"==colnames(test)))]
