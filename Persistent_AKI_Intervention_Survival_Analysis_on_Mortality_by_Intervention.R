...
# setting
analysis="_intervention_on_mortality_by_each"
...
for (i in 1:length(impDat)){
  ...
  if(analysis == "_intervention_on_mortality_by_each"){
    data = data %>%
  ...
  for (int in 1:length(int_var)){
    ...
    coxdata = rawData %>%
      ...
      select(all_of(adj_var), all_of(int_var[int]))
    names(coxdata)[length(names(coxdata))] = "Intervention"
