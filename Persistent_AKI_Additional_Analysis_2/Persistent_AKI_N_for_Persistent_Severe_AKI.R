...
N = vector()
N_psAKI = vector()
N_stage23_first_psAKI = vector()
N_stage123_psAKI = vector()
...
for (i in 1:length(impDat)){
  ...
  N_psAKI[i] = psAKI_data %>%
    nrow()
  
  N_stage23_first_psAKI[i] = psAKI_data %>%
    filter(FIRST_AKI_STAGE == 2 | FIRST_AKI_STAGE == 3) %>%
    nrow()
  
  N_stage123_psAKI[i] = psAKI_data %>%
    filter(day7_kidgo>0) %>%
    nrow()
...
mean(N_stage23_first_psAKI)/mean(N_psAKI) * 100
mean(N_stage123_psAKI)/mean(N_psAKI) * 100

sum(is.na(data$AKI_Category_subgroup2))
