...
for (pattern in pattern_all){
  result = mapply(compare_patterns, pattern = pattern, text = rawData$KDIGO_pattern_first3)
...
min_position = apply(result_df, 1, get_min_position)
min_value = apply(result_df, 1, get_min_value)
min_position_df = tibble(min_position=min_position, 
                         min_value = min_value)

min_position_df = min_position_df %>%
  mutate(subgroup_new = ifelse((min_position<=16 & min_value!=999), "Persistent AKI stage 1", 
                               ifelse((min_position>16 & min_position<=32 & min_value!=999), "Persistent AKI stage 2",
                                      ifelse((min_position>32 & min_position<=48 & min_value!=999), "Persistent AKI stage 3",
                                             ifelse((min_position>48 & min_position<=75 & min_value!=999), "Transient_AKI", NA)))))
